...
const PRIMARY_OPPORTUNITY_LABEL = {
...
function dominantSegment(segments) {
    if (segments.length === 0)
        return null;
    return segments.reduce((largest, segment) => segment.bytes > largest.bytes ? segment : largest);
}
...
const INSIGHT_TEMPLATES = [
    ...
    },
    {
        ...
        when: (ctx) => {
            ...
            if (!query)
                return false;
            return (contributorShare(query.responseBytes, ctx.view.byUrlKind.query.responseBytes) >= CONCENTRATION_SHARE);
        },
        ...
        score: (ctx) => {
            ...
            if (!query)
                return 0;
            return (contributorShare(query.responseBytes, ctx.view.byUrlKind.query.responseBytes) * 80);
        },
    },
    {
        kind: "fact",
        when: (ctx) => {
            ...
            if (!image)
                return false;
            return (contributorShare(image.responseBytes, ctx.view.byUrlKind.image.responseBytes) >= CONCENTRATION_SHARE);
        },
        ...
        score: (ctx) => {
            ...
            if (!image)
                return 0;
            return (contributorShare(image.responseBytes, ctx.view.byUrlKind.image.responseBytes) * 70);
        },
    },
    {
        kind: "opportunity",
    ...
    {
        kind: "health",
        when: (ctx) => ctx.summary.critical.length === 0 &&
...
export function buildAtAGlance(view, summary) {
    const ctx = {
        view,
        summary,
    ...
    for (const item of bullets) {
        if (selected.length >= 4)
            break;
        if (item.kind !== "health" && usedKinds.has(item.kind))
            continue;
