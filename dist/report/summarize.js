...
function toOpportunity(finding) {
    if (finding.severity === "passed" || !finding.suggestedFix)
        return null;
    return {
...
export function buildReportSummary(view) {
    ...
    if (imageRows.length > 0) {
        ...
        const unsafeRequests = unsafeFormatRows.reduce((sum, { row }) => sum + row.requests, 0);
        const totalImageRequests = imageRows.reduce((sum, { row }) => sum + row.requests, 0);
    ...
    const addConcentrationFinding = (id, title, rows, dimensionLabel) => {
        const byBandwidth = dominantRankedRow(rows, "responseBytes");
        const dominant = byBandwidth && byBandwidth.share >= CONCENTRATION_SHARE_THRESHOLD
            ? { ...byBandwidth, metricLabel: "bandwidth" }
            : (() => {
                ...
                return { ...byRequests, metricLabel: "requests" };
            })();
        if (!dominant)
            return;
        const severity = dominant.share >= 0.9 ? "critical" : "warning";
        pushFinding(findings, {
            id,
            severity: severity,
            title,
            ...
            requests: dominant.metricLabel === "requests" ? dominant.value : undefined,
            responseBytes: dominant.metricLabel === "bandwidth" ? dominant.value : undefined,
        });
    };
    addConcentrationFinding("hour-concentration", "Hourly traffic concentration", view.byHour, "hour");
    addConcentrationFinding("domain-concentration", "Domain traffic concentration", view.byDomain, "domain");
    addConcentrationFinding("endpoint-concentration", "Endpoint traffic concentration", view.byEndpoint, "endpoint");
    ...
    const estimatedSavingsBytes = topOpportunities.reduce((sum, item) => {
        if (item.estimatedSavingsBytes === undefined)
            return sum;
